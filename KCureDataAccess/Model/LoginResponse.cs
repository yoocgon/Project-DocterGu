    ...
    public class LoginResult
    {
        ...
        public List<Data> GetUserValidDatas(bool forCarry)
        {
            ...
            foreach (var item in authCodes)
            {
                ...
                if (authCode != CCommon.Const.USER_AU006) continue;

                var datas = item.Value;
                foreach (var data in datas)
                {
                    if (string.IsNullOrEmpty(data.dtuSdt)) continue;
                    if (string.IsNullOrEmpty(data.dtuEdt)) continue;

                    if (!forCarry && string.IsNullOrEmpty(data.prtiFile)) continue;
                    if (forCarry && string.IsNullOrEmpty(data.carryFile)) continue;
